...
function NetworkSelect(props) {
  ...
  return (
    <>
      <Button onClick={handleOpen} variant="link" className="d-flex align-items-center text-dark text-decoration-none border-secondary btn-outline-light mr-4" role="button">
        <div className="avatar avatar-sm rounded-circle text-dark">
          <img alt={props.network.prettyName} src={props.network.data.image} height={30} width={30} />
        </div>
        <div className="d-none d-sm-block ms-2">
          <span className="network h6">{props.network.prettyName}</span>
        </div>
        <div className="d-none d-sm-block ms-2">
                  ...
                  formatOptionLabel={network => (
                    <div className="row">
                      ...
                      <div className="col text-end pt-1">
                        ...
                        <small><Badge className="ms-3 rounded-pill" bg="success">ShapeShift Validator Online</Badge></small>
                        }
